...
Inherited numeric prototype entries are ignored during bounded fallback scans.
...
When iterator cloning succeeds, disjoint key-scan supplementation
remains uncapped so non-overlapping recovered entries are preserved.
Step/check aggregate totals are clamped to at least partition counts
when uncapped supplementation adds disjoint recovered entries.
...
key-scan string recovery supplements missing string entries by index
while preserving already recovered string entries.
