...
Ƭ **ArmsOptions**: [`CanvasBoxOptions`](modules.md#canvasboxoptions-6) & { `shoulderDrop?`: `number` ; `shoulderGap?`: `number`  }
...
| `dynamicFn` | (`pos`: [`Coords3`](modules.md#coords3-18), `world`: [`World`](classes/World.md)) => { `aabbs`: [`Block`](modules.md#block-18)[``"aabbs"``] ; `faces`: [`Block`](modules.md#block-18)[``"faces"``] ; `isTransparent`: [`Block`](modules.md#block-18)[``"isTransparent"``]  } | If this block is dynamic, this function will be called to generate the faces and AABB's. By default, this just returns the faces and AABB's that are defined in the block data. |
| `faces` | { `corners`: { `pos`: `number`[] ; `uv`: `number`[]  }[] ; `dir`: `number`[] ; `independent`: `boolean` ; `name`: `string` ; `range`: [`TextureRange`](modules.md#texturerange-18)  }[] | A list of block face data that this block has. |
...
Ƭ **BodyOptions**: [`CanvasBoxOptions`](modules.md#canvasboxoptions-6)
...
| Name | Type | Description |
| :------ | :------ | :------ |
| `arms?` | `Partial`<[`ArmsOptions`](modules.md#armsoptions-6)\> | Parameters to create the character's arms. |
| `body?` | `Partial`<[`BodyOptions`](modules.md#bodyoptions-6)\> | Parameters to create the character's body. |
| `head?` | `Partial`<[`HeadOptions`](modules.md#headoptions-6)\> | Parameters to create the character's head. |
| `idleArmSwing?` | `number` | The speed at which the arms swing when the character is idle. Defaults to `0.06`. |
| `legs?` | `Partial`<[`LegOptions`](modules.md#legoptions-6)\> | Parameters to create the character's legs. |
...
| `dimensions` | [`Coords3`](modules.md#coords3-18) | The dimension of each cloud block. Defaults to `[20, 20, 20]`. |
...
| `onByDefault` | `boolean` | Whether or not should the debug panel be displayed by default when the page loads. Defaults to `true`. You can toggle the debug panel by calling [toggle](classes/Debug.md#toggle-18). |
...
Ƭ **HeadOptions**: [`CanvasBoxOptions`](modules.md#canvasboxoptions-6) & { `neckGap?`: `number`  }
...
| `occasion?` | [`InputOccasion`](modules.md#inputoccasion-18) | The occasion that the input should be fired. Defaults to `keydown`. |
...
Ƭ **LegOptions**: [`CanvasBoxOptions`](modules.md#canvasboxoptions-6) & { `betweenLegsGap?`: `number`  }
...
| `perspective` | [`CameraPerspective`](modules.md#cameraperspective-18) | The position of where the camera should be looking at. Defaults to `pxyz`, which means that the camera will be looking at the center of the object from the positive x, y, and z axis scaled by the zoom. |
...
| `cloudsOptions` | `Partial`<[`CloudsOptions`](modules.md#cloudsoptions-6)\> | The options to create the clouds. Defaults to `{}`. |
...
| `skyOptions` | `Partial`<[`SkyOptions`](modules.md#skyoptions-6)\> | The options to create the sky. Defaults to `{}`. |
...
Ƭ **WorldOptions**: [`WorldClientOptions`](modules.md#worldclientoptions-6) & [`WorldServerOptions`](modules.md#worldserveroptions-6)

The options to create a world. This consists of [WorldClientOptions](modules.md#worldclientoptions-6) and [WorldServerOptions](modules.md#worldserveroptions-6).
...
• `Const` **BOX\_SIDES**: [`BoxSides`](modules.md#boxsides-18)[]
...
| Name | Type |
| :------ | :------ |
| `drawCrown` | [`ArtFunction`](modules.md#artfunction-18) |
