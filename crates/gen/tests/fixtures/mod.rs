...
/// A small, fast geology-lane world over the fixture registry: analytic
/// plates and a short erosion solve, drainage rivers, a ground mosaic,
/// and a two-community ecology with named-species canopies. Numbers are
/// sized for a 128-block world and test-speed tile solves.
pub fn geology_fixture_spec() -> GeneratorSpec {
    let temperature = {
        let mut b = FieldGraphBuilder::new();
        b.fbm("geofix.temperature", 1.0 / 400.0, 2, 0.5, 2.0);
        b.build()
    };
    GeneratorSpec {
        preset: "geology_fixture",
        format_version: FORMAT_VERSION,
        content_version: Version::new(1, 0, 0),
        dimension: DimensionSpec {
            key: "geology_fixture_dim",
            height: HEIGHT as u32,
            base_block: "Test Stone",
            sky_presentation_key: "default",
            capabilities: DimCapabilities {
        ...
        topology: TopologySpec::Geology(GeologySpec {
        ...
        rivers: Some(RiverSpec {
            materials: RiverMaterials {
                water: "Test Water",
                bed: "Test Cobble",
                bank: "Test Sand",
            },
            routing: RiverRouting::Drainage,
        }),
        ecology: Some(EcologySpec {
        ...
        biomes: BiomeSetSpec {
            ...
            partition: BiomePartition::Single(BiomeKey("meadow")),
            overlays: vec![],
        },
        surface: SurfaceSpec {
            tables: vec![(
                ...
                SurfaceTable {
                    rules: vec![
                        ...
                        SurfaceRule {
                            when: vec![
                                SurfaceCond::DepthBelowTop { min: 0, max: 1 },
                                SurfaceCond::IsUnderFluid,
                            ],
                            place: "Test Sand",
                        },
                        SurfaceRule {
...
/// The heightfield fixture with the 3D density spine engaged: cliff-band
/// candidacy from its own mask field, strata bias for shelves, and a
/// coastal notch.
pub fn density_fixture_spec() -> GeneratorSpec {
    let mut spec = fixture_spec();
    let mask = {
        let mut b = FieldGraphBuilder::new();
        let bands = b.fbm("fixture.density.bands", 1.0 / 90.0, 2, 0.5, 2.0);
        b.gate(bands, 0.01, 0.09);
        b.build()
    };
    spec.density = Some(DensitySpec {
        salt: SaltPath("fixture.density"),
        amplitude: 5.0,
        frequency: 1.0 / 26.0,
        octaves: 2,
        y_squash: 0.6,
        mask,
        strata: Some(DensityStrata {
            period: 7.0,
            contrast: 0.5,
        }),
        coastal_notch: 2,
...
pub fn walker_fixture_spec() -> GeneratorSpec {
    ...
    spec.rivers = Some(RiverSpec {
        ...
        routing: RiverRouting::Walker(WalkerRivers {
...
pub fn harness_for(spec: GeneratorSpec) -> Harness {
    ...
    let is_river_world = spec.rivers.is_some();
    let is_flora_world = !spec.flora.is_empty() || spec.ecology.is_some();
    ...
    if is_river_world {
        stages.push(Box::new(stages::GenRiverStage::new(Arc::clone(&generator))));
    }
    if is_flora_world {
        stages.push(Box::new(stages::GenFloraStage::new(Arc::clone(&generator))));
    }
    Harness {
        registry,
        config,
