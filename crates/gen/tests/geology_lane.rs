//! The geology lane through the full public pipeline: chunk-order and
...
//! and a generation cost smoke. The fixture is a small, fast solve — the
//! physics fidelity tests live with the solver's unit suite.
...
fn geology_tile_digests_are_deterministic() {
    ...
    assert!(
        ...
        "the geology lane must answer tile digests"
    );
}
...
fn mosaic_paints_the_geo_surface() {
    let harness = harness_for(geology_fixture_spec());
    let counts = |name: &str| harness.registry.get_block_by_name(name).id;
    let dry = counts("Test Dry Grass");
    let lush = counts("Test Lush Grass");
    let snow = counts("Test Snow");
    let dirt = counts("Test Dirt");
        ...
        for cz in -6..6 {
            ...
            for x in min_x..max_x {
                for z in min_z..max_z {
                    ...
                    let y = surface.clamp(min_y, max_y - 1);
                    let top = chunk.get_voxel(x, y, z);
                    if top == dry || top == lush {
                        tone_blocks += 1;
                    }
                    if top == snow {
