...
fn walker_water_levels_are_monotone_downstream() {
    ...
    let TopologySpec::Heightfield(_) = walker_fixture_spec().topology else {
        unreachable!("the walker fixture runs the heightfield lane");
    };
    let Some(RiverSpec {
        routing: RiverRouting::Walker(walker),
        ..
    }) = walker_fixture_spec().rivers
    else {
        unreachable!("the walker fixture carries walker rivers");
    };
    let rivers = CompiledWalkerRivers::compile(
        &walker,
        generator.sea_level(),
        SEED,
        "fixture_dim",
...
fn community_species_never_leak_their_patches() {
    ...
    let species_key = |index: usize| walker_fixture_spec().species[index].key;
    let mut checked = 0usize;
    for tree in &trees {
        let key = species_key(tree.species);
        if key == "snag" {
...
fn river_and_flora_specs_validate() {
    ...
    let mut drainage_on_heightfield = walker_fixture_spec();
    if let Some(rivers) = &mut drainage_on_heightfield.rivers {
        rivers.routing = RiverRouting::Drainage;
    }
    assert!(matches!(
        compile(&drainage_on_heightfield, &registry, &config),
        Err(GenError::Invalid { .. })
    ));

    let mut walker_on_geology = geology_fixture_spec();
    if let Some(rivers) = &mut walker_on_geology.rivers {
        rivers.routing = RiverRouting::Walker(WalkerRivers {
            salt: SaltPath("geofix.walker"),
            tile: 256,
            sources_per_tile: 2,
            min_source_height: 70,
            max_steps: 100,
            width: (1.0, 3.0),
            depth: (1.0, 2.0),
            bank: 2.0,
    ...
    assert!(matches!(
        compile(&walker_on_geology, &registry, &config),
        Err(GenError::Invalid { .. })
    ));
    ...
    if let Some(rivers) = &mut tiny_tile.rivers {
        rivers.routing = RiverRouting::Walker(WalkerRivers {
            salt: SaltPath("fixture.rivers.tiny"),
            tile: 64,
            sources_per_tile: 2,
            min_source_height: 56,
            max_steps: 100,
            width: (1.0, 3.0),
            depth: (1.0, 2.0),
            bank: 2.0,
    ...
    assert!(matches!(
        compile(&tiny_tile, &registry, &config),
        Err(GenError::Invalid { .. })
    ));

    let mut unknown_water = walker_fixture_spec();
    if let Some(rivers) = &mut unknown_water.rivers {
        rivers.materials.water = "No Such Fluid";
    }
    assert!(matches!(
    ...
    assert!(matches!(
        compile(&unknown_species, &registry, &config),
        Err(GenError::Invalid { .. })
    ));
    ...
    assert!(matches!(
        compile(&bad_ecotone, &registry, &config),
        Err(GenError::Invalid { .. })
    ));

    let mut geo_without_sea = geology_fixture_spec();
    geo_without_sea.hydrology.sea = None;
    assert!(matches!(
        compile(&geo_without_sea, &registry, &config),
        Err(GenError::Invalid { .. })
    ));

    let mut tall_ceiling = geology_fixture_spec();
    if let TopologySpec::Geology(geology) = &mut tall_ceiling.topology {
        geology.ceiling_max = 200.0;
    }
    assert!(matches!(
        compile(&tall_ceiling, &registry, &config),
        Err(GenError::Invalid { .. })
    ));

    let mut bad_mosaic = geology_fixture_spec();
    if let Some(mosaic) = &mut bad_mosaic.mosaic {
        mosaic.grass_block = "No Such Grass";
    }
    assert!(matches!(
        compile(&bad_mosaic, &registry, &config),
        Err(GenError::UnknownBlock { .. })
    ));
}
