...
If a split value position is followed by a recognized option token (for example `--output -l`), that token is treated as an option and the value is considered missing.
