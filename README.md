...
If a split `--output` or `--only` value position is followed by a recognized option token (for example `--output -l` or `--only -l`) or recognized inline option misuse (for example `--output -l=1`), that token is treated as an option and the value is considered missing.
