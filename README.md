...
runtime-library JSON preflight reports additionally include `packagesPresent`, `checkedPackages`, `checkedPackageCount`, `packageReports`, `requiredPackageCount`, `presentPackageCount`, `packageReportCount`, `requiredArtifactCount`, `missingPackageCount`, `missingArtifactCount`, `buildCommand`, `buildArgs`, `buildExitCode`, `buildDurationMs`, `attemptedBuild`, `buildSkipped`, and `buildSkippedReason` to classify multi-package artifact readiness and auto-build behavior.
