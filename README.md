...
For ts-core reports, `examplePayloadValid` (and aggregate aliases like `tsCoreExamplePayloadValid`) is `true` only when the example output includes a complete payload (`voxel`, `light`, and `rotatedAabb`) with ordered bounds (`min <= max` per axis).
