...
Client and onboarding JSON reports also include `availableSteps`, `availableStepCount`, `totalSteps`, `passedStepCount`, `failedStepCount`, `skippedStepCount`, `firstFailedStep`, `passedSteps`, `failedSteps`, and `skippedSteps`.
...
Aggregate preflight reports include `passedCheckScripts`, `passedCheckScriptCount`, `failedCheckScripts`, and `failedCheckScriptCount` so CI systems can map pass/fail status directly to executed scripts.
