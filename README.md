...
Aggregate preflight reports include `selectedCheckMetadata`, `selectedCheckMetadataCount`, `selectedCheckScripts`, and `selectedCheckScriptCount` as well as `skippedCheckMetadata`, `skippedCheckMetadataCount`, `skippedCheckScripts`, and `skippedCheckScriptCount` so automation can map selected/skipped checks directly to executable scripts.
Aggregate preflight reports include `passedCheckScripts`, `passedCheckScriptCount`, `failedCheckScripts`, and `failedCheckScriptCount` plus `passedCheckMetadata`, `passedCheckMetadataCount`, `failedCheckMetadata`, `failedCheckMetadataCount`, `passedCheckIndices`/`passedCheckIndexCount`, `passedCheckIndexMap`/`passedCheckIndexMapCount`, `failedCheckIndices`/`failedCheckIndexCount`, and `failedCheckIndexMap`/`failedCheckIndexMapCount` so CI systems can map pass/fail status directly to executed scripts.
