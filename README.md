...
When `ruleMatched=false` is reported together with payload invalidity, ts-core failure messages include both signals (rule mismatch + payload issue paths) for faster triage.
