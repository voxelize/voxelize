...
Each `steps[]` entry in client/onboarding JSON mode includes `scriptName` and `stepIndex` so downstream tooling can map step outcomes to stable script identifiers.
...
Each `checks[]` entry in aggregate preflight reports includes `scriptName` and `checkIndex` so execution records can be correlated with canonical check metadata.
