...
ts-core JSON preflight reports additionally include `artifactsPresent`, `missingArtifacts`, `requiredArtifactCount`, `missingArtifactCount`, `buildCommand`, `buildArgs`, `attemptedBuild`, and `buildSkipped` to classify artifact readiness and auto-build behavior.
