...
For ts-core reports, `examplePayloadValid` (and aggregate aliases like `tsCoreExamplePayloadValid`) is `true` only when the example output includes a complete payload (`voxel.id`, `voxel.stage`, `voxel.rotation.value`, `voxel.rotation.yRotation`, `light`, and `rotatedAabb`) with ordered bounds (`min <= max` per axis).
