...
fn test_mesh_empty_space() {
    ...
    let space = chunks.make_space(&coords, 1).needs_voxels().build();
    ...
    assert!(
        geometries.is_empty(),
        "Empty space should produce no geometry"
    );
}
...
fn test_mesh_single_block() {
    ...
    assert!(
        !geometries.is_empty(),
        "Single block should produce geometry"
    );
    assert_eq!(
        geometries.len(),
        1,
        "Should have one geometry for stone block"
    );
...
fn test_mesh_surrounded_block() {
    ...
    assert!(
        total_vertices > 0,
        "Should generate some vertices for exposed faces"
    );
}
...
fn test_mesher_add_remove_chunk() {
    ...
    assert_eq!(
        got,
        Some(coords.clone()),
        "Should have chunk in queue after adding"
    );
...
fn test_mesh_produces_valid_geometry() {
    ...
    for geometry in geometries {
        assert_eq!(
            ...
            0,
            "Positions should be divisible by 3"
        );
        ...
        assert_eq!(
            geometry.positions.len() / 3,
            geometry.uvs.len() / 2,
            "Positions and UVs should match"
        );
        assert_eq!(
            geometry.positions.len() / 3,
            geometry.lights.len(),
            "Positions and lights should match"
        );
    }
}
