...
fn batch_messages(messages: Vec<(Message, ClientFilter)>) -> Vec<(Message, ClientFilter)> {
    ...
    let mut result: Vec<(Message, ClientFilter)> = if let Some(unbatched) = unbatched {
        let mut combined = Vec::with_capacity(batched.len() + unbatched.len());
        for ((_, filter), message) in batched {
            combined.push((message, filter));
        }
        combined.extend(unbatched);
        return combined;
    } else {
                ...
                ClientFilter::Exclude(ids) => {
                    if ids.is_empty() {
                        ...
                        match ids.as_slice() {
                            ...
                            [first_id, second_id, third_id, fourth_id, fifth_id, sixth_id, seventh_id, eighth_id, ninth_id] => {
                                let first_id = first_id.as_str();
                                let second_id = second_id.as_str();
                                let third_id = third_id.as_str();
                                let fourth_id = fourth_id.as_str();
