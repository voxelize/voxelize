...
interface GenericRenderer {
  ...
  clear(): void;
  render(scene: Scene | Object3D, camera: Camera): void;
}
...
export class CSMRendererCompat {
  ...
  private initCascades() {
    ...
    for (let i = 0; i <= cascades; i++) {
      const p = i / cascades;
      splits.push(Math.pow(p, lambda) * maxShadowDistance);
    }
  ...
  render(
    ...
    const poolOriginalMaterials: Map<Object3D, Mesh["material"]> = new Map();
    if (this.shouldRenderEntityShadows && instancePools) {
      for (const pool of instancePools) {
        pool.traverse((child) => {
          if (child instanceof Mesh && child.customDepthMaterial) {
    ...
    for (const [mesh, originalMaterial] of poolOriginalMaterials) {
      (mesh as Mesh).material = originalMaterial;
    }
  ...
  markCascadesForEntityRender() {
    ...
    if (!this.shouldRenderEntityShadows) return;
