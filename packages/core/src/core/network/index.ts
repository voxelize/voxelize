...
/**
 * Transport type for network connection.
 * - "webrtc": Use WebRTC DataChannels for UDP-like unreliable transport (lower latency)
 * - "websocket": Use WebSocket for reliable TCP-based transport
 */
...
export type NetworkConnectionOptions = {
  ...
  /**
   * The transport protocol to use. Defaults to "webrtc" which provides lower latency.
   * Falls back to "websocket" if WebRTC connection fails.
   */
...
export class Network {
  ...
  /**
   * The WebRTC connection for UDP-like unreliable transport.
   */
  ...
  /**
   * Connect to a Voxelize server. Defaults to WebRTC for lower latency, with automatic
   * fallback to WebSocket if WebRTC fails. Remember to set username and ID before connection
   * if you want to specify them manually. Otherwise ID is generated by the server, and username
  ...
  connect = async (
    ...
    if (transport === "webrtc") {
      try {
        return await this.connectWebRTC(serverURL, options);
      } catch (e) {
        console.warn(
          "[NETWORK] WebRTC connection failed, falling back to WebSocket:",
          e
        );
        return await this.connectWebSocket(serverURL, options);
      }
    }
  ...
  /**
   * Connect to a Voxelize server using WebSocket for reliable TCP-based transport.
   *
   * @param serverURL The URL to the Voxelize server.
   * @param options Parameters to customize the connection.
   * @returns A promise that resolves when the client has connected to the server.
   */
  ...
  /**
   * Connect to a Voxelize server using WebRTC DataChannels for UDP-like unreliable transport.
   * This provides lower latency compared to WebSocket but messages may be lost or arrive out of order.
   *
   * @param serverURL The URL to the Voxelize server (http:// or https://).
   * @param options Parameters to customize the connection.
   * @returns A promise that resolves when the client has connected to the server.
   */
  connectWebRTC = async (
    ...
    return new Promise<Network>((resolve, reject) => {
      this.rtc = new WebRTCConnection(httpUrl, {
      ...
      this.rtc.onClose = () => {
        console.log("[NETWORK] WebRTC connection closed");
        this.connected = false;
        this.onDisconnect?.();

        if (options.reconnectTimeout) {
          this.reconnection = setTimeout(() => {
            this.connectWebRTC(serverURL, options);
          }, options.reconnectTimeout);
        }
      };
      ...
      this.rtc.onMessage = (data: ArrayBuffer) => {
        if (this.waitingForInit) {
          if (!this.initPacketReceived) {
            this.initPacketReceived = true;
            const bufferCopy = new Uint8Array(data.slice(0));
            this.decodePriority(bufferCopy, data);
          } else {
      ...
      this.rtc
        .connect(this.clientInfo.id || undefined)
        .then((clientId) => {
  ...
  /**
   * The type of transport being used for the network connection.
   * Returns "webrtc" if using WebRTC DataChannels, "websocket" if using WebSocket.
   */
  get transportType(): "webrtc" | "websocket" {
    return this.rtc?.connected ? "webrtc" : "websocket";
  }
