...
describe("Type builders", () => {
  ...
  it("salvages keyed transparency arrays in conditional parts", () => {
    const keyedTransparency = new Proxy([true, false, true, false, true, false], {
      get(target, property, receiver) {
        if (property === Symbol.iterator) {
          return function* () {
    ...
    const part = createBlockConditionalPart({
      isTransparent: keyedTransparency as never,
    });
  ...
  it("normalizes explicit-empty iterator transparency arrays in conditional parts", () => {
    ...
    const part = createBlockConditionalPart({
      isTransparent: explicitEmptyTransparency as never,
    });
  ...
  it("salvages empty-iterator face and aabb entries during conditional part cloning", () => {
  ...
  it("sanitizes empty-iterator ownKeys-trapped face and aabb entries", () => {
    const trappedFaces = new Proxy([{ name: "IteratorFace" }], {
      ownKeys() {
        throw new Error("ownKeys trap");
      },
  ...
  it("prefers key-fallback defined face entries over bounded undefined placeholders", () => {
    let prefixReadCount = 0;
    const sparseFaces: BlockFaceInit[] = [];
    sparseFaces[0] = { name: "PrefixFace" };
  ...
  it("salvages empty-iterator combination rule entries", () => {
  ...
  it("salvages empty-iterator dynamic pattern part entries", () => {
  ...
  it("sanitizes empty-iterator ownKeys-trapped dynamic pattern part entries", () => {
    const trappedParts = new Proxy([{ worldSpace: true }], {
      ownKeys() {
        throw new Error("ownKeys trap");
      },
  ...
  it("normalizes ownKeys-trapped empty-iterator corner collections in createBlockFace", () => {
    const trappedCorners = new Proxy(
      [
        createCornerData([0, 0, 0], [0, 0]),
        createCornerData([1, 0, 0], [1, 0]),
...
describe("BlockRuleEvaluator", () => {
  ...
  it("salvages indexed combination rules when iterators return empty entries", () => {
    const access = {
      getVoxel: (x: number, y: number, z: number) =>
        x === 0 && y === 0 && z === 0 ? 37 : 0,
      getVoxelRotation: () => BlockRotation.py(0),
    ...
    const emptyIteratorRules = new Proxy(
      [
        {
          type: "simple" as const,
          offset: [0, 0, 0] as [number, number, number],
      ...
      BlockRuleEvaluator.evaluate(
        {
          type: "combination",
          logic: BlockRuleLogic.Or,
          rules: emptyIteratorRules as never,
        },
        [0, 0, 0],
        access
      )
